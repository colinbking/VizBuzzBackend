...
import wave
import json
# from ..Transcriber.transcriber import vz_speech_recog

frame_from_ns = lambda x: int((((x * 10**-4) / 1000) * 16000))

def add_pitch_to_output(speechobj, output_format:dict, cut = 5, plot = False):
    ...
    for idx, tes in enumerate(med_output):
        ...
        # print(f"inside loop, {idx}, {tes}")
        _ = speechobj.rigged_format.readframes(frame_from_ns(tes['Offset'] - speechobj.last_o_d_global))
        ...
        frames_to_process = speechobj.rigged_format.readframes(frame_count)
        ...
        speechobj.last_o_d_global = tes['Duration'] + tes['Offset']
    if plot:
